{{-- ── Transfer Modal — include inside your @foreach loop ─── --}}
        ...
        <div class="modal-content transfer-modal">
            ...
            <div class="modal-header border-0 px-4 pt-4 pb-0">
                <div class="d-flex align-items-center gap-2">
                    ...
                    <h5 class="modal-title fw-bold mb-0 text-dark">Stock Transfer</h5>
                </div>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>

            <form action="{{ route('wholesale_storage.transfer', $item->id) }}" method="POST">
                @csrf
                <div class="modal-body px-4 pt-4 pb-2">
                    ...
                    <div class="med-pill">
                        ...
                        <div class="flex-grow-1 min-w-0">
                            <div class="fw-bold text-dark text-truncate">{{ $item->medicine->name }}</div>
                            <div class="text-muted small">Batch: #{{ $item->batch_number ?? 'N/A' }}</div>
                        </div>
                        <span class="avail-badge">{{ $item->quantity }} units</span>
                    </div>
                    ...
                    <div class="qty-section">
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            <label class="qty-label">Quantity to Transfer</label>
                            <span class="max-hint">Max: <strong>{{ $item->quantity }}</strong></span>
                        </div>
                        <div class="qty-wrap" id="qtyWrap{{ $item->id }}">
                            <input
                                ...
                                max="{{ $item->quantity }}"
                                required
                                oninput="checkQty(this, {{ $item->quantity }}, '{{ $item->id }}')"
                            >
                            <span class="qty-suffix">units</span>
                        </div>
                        <div class="qty-error d-none" id="qtyError{{ $item->id }}">
                            <i class="bi bi-exclamation-triangle me-1"></i>
                            Cannot exceed {{ $item->quantity }} available units
                        </div>
                    </div>
                    ...
                    <div class="transfer-note">
                        ...
                        <span>Stock will be moved to <strong>Pharmacy Retail Storage</strong> immediately and cannot be undone.</span>
                    </div>
                ...
                <div class="modal-footer border-0 px-4 pt-2 pb-4 gap-2">
                    ...
                    <button type="submit" class="btn-confirm" id="confirmBtn{{ $item->id }}">
                        <i class="bi bi-check2-circle me-2"></i>Confirm Transfer
                    </button>
                </div>
            </form>
        </div>
...
.transfer-bar {
    height: 4px;
    background: linear-gradient(90deg, #107A84 0%, #0d9488 100%);
}
.transfer-header-icon {
    width: 34px; height: 34px;
    background: #107A84; border-radius: 9px;
...
.med-pill-icon {
    ...
    background: rgba(16,122,132,0.1); color: #107A84;
    border-radius: 11px;
    display: flex; align-items: center; justify-content: center;
    font-size: 1.1rem;
}
...
.flow-node.from { background: rgba(16,122,132,0.08); color: #107A84; border: 1.5px solid rgba(16,122,132,0.18); }
...
.flow-line {
    flex: 1; height: 2px;
    background: linear-gradient(90deg, rgba(16,122,132,0.2), rgba(16,185,129,0.2));
}
.flow-icon { font-size: 1.1rem; color: #107A84; flex-shrink: 0; }
...
.transfer-note {
    ...
    background: rgba(16,122,132,0.06);
    border: 1px solid rgba(16,122,132,0.14);
    border-radius: 11px; padding: 11px 14px;
    font-size: 0.8rem; color: #107A84; line-height: 1.45;
}
.btn-cancel {
...
.btn-confirm:hover {
    ...
    box-shadow: 0 4px 14px rgba(16,122,132,0.3);
}
.btn-confirm:disabled { opacity: 0.5; cursor: not-allowed; transform: none; }
</style>

<script>
function checkQty(input, max, id) {
    ...
    const over  = parseInt(input.value) > max;
