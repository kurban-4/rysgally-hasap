...
<div class="desktop-app-layout">
    ...
    <main class="app-main bg-mesh">
        <header class="main-header d-print-none">
            ...
            <div>
                <h4 class="fw-bold mb-0" style="color:#107A84;">Invoice #{{ $invoice->invoice_no }}</h4>
            ...
            <div class="ms-auto d-flex gap-2">
                <a href="{{ route('wholesale.edit', $invoice->id) }}" class="btn-teal">
                    <i class="bi bi-pencil-square me-1 me-md-2"></i><span class="d-none d-md-inline">Edit</span>
                </a>
                <button onclick="window.print()" class="btn-dark-sm">
                    <i class="bi bi-printer me-1 me-md-2"></i><span class="d-none d-md-inline">Print</span>
                </button>
            </div>
        </header>
                ...
                <div class="receipt-card">
                    ...
                    <div class="receipt-body">
                        <div class="inv-header-grid">
                            <div>
                                <div class="brand-row"><i class="bi bi-capsule me-2"></i>PHARMA<span style="color:#107A84;">SYS</span></div>
                                <p class="text-muted small mb-0">Official Wholesale Department</p>
                            </div>
                        ...
                        <div class="customer-box">
                            <div>
                                ...
                                <h5 class="fw-bold mb-0" style="color:#107A84;">{{ $invoice->customer_name }}</h5>
                                <small class="text-muted">Registered Wholesale Partner</small>
                            </div>
                            <div class="text-sm-end">
                                <div class="box-label">Status</div>
                                <span class="paid-badge"><i class="bi bi-check-circle-fill me-1"></i>PAID & PROCESSED</span>
                            </div>
                        </div>
                        <div class="table-responsive mb-4">
                            <table class="table inv-table align-middle">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td class="fw-bold text-dark">{{ $item->medicine->name }}</td>
                        ...
                        <div class="totals-block">
                            <div class="total-row"><span class="text-muted small">Subtotal:</span><span class="fw-bold">${{ number_format($invoice->items->sum(fn($i)=>$i->quantity*$i->unit_price),2) }}</span></div>
                            <div class="total-row"><span class="text-muted small">Discount:</span><span class="fw-bold text-danger">-${{ number_format($invoice->items->sum(fn($i)=>($i->quantity*$i->unit_price)-$i->row_total),2) }}</span></div>
                            <div class="total-divider"></div>
                            <div class="grand-row"><span class="fw-bold text-uppercase small">Grand Total</span><span class="grand-val">${{ number_format($invoice->total_amount,2) }}</span></div>
                        </div>

                        <div class="text-center mt-5 opacity-25 small fw-bold text-uppercase">Thank you for your business!</div>
                    </div>
                </div>
            </div>
        </div>
...
.btn-teal { background:#107A84; color:white; border:none; border-radius:10px; padding:8px 16px; font-weight:700; font-size:0.82rem; display:flex; align-items:center; text-decoration:none; transition:0.2s; }
...
.receipt-top-bar { height:8px; background:linear-gradient(90deg,#107A84,#0d636b); }
...
.grand-row { display:flex; justify-content:space-between; align-items:center; background:#107A84; color:white; padding:12px 16px; border-radius:12px; }
